<template>
  <el-dialog title="编辑文档" v-if="Visible" :visible.sync="Visible" width="490px" top="5vh" :show-close="false" :close-on-click-modal="false">
    <div style="width: 80%">
      文档标题:<br><br>
      <el-input placeholder="请输入标题" v-model="OldFile.Title"></el-input>
    </div>
    <br>
    <div>文档简介:</div>
    <br>
    <el-input placeholder="请输入文档简介" :rows="5" v-model="OldFile.SimpleMessage" style="width: 80%" type="textarea"></el-input>
    <br><br>
    <div>
      <div>权限:</div>
      <div>
        <el-select v-model="OldFile.Authority">
          <el-option v-for="(i,index) in Authority" :label="i" :value="index">{{i}}</el-option>
        </el-select>
        <el-select v-model="OldFile.Revise" v-if="OldFile.Authority!==0">
...
<script>
  ...
  export default {
    name: "ConfigOldFile",
    props:['Visible','Title','SimpleMessage','ArticleAuthority','Revise','aid'],
    data(){
      return {
    ...
    methods:{
      submit(){
        ...
        axios.get('http://127.0.0.1:8000/judgeRepetitiveArticleName',{params:{
          ...
          this.$router.push({path:'/tools/editfile',query:{OldFile:this.OldFile}})
        }).catch(res=>{
